...
const HuddleReport = ({ drawerWidth, isLoading }) => {
   ...
   const scorecardColumns = React.useMemo(
      () => [
         ...
         { Header: 'Actual', accessor: 'actual', width: 100, Footer: '' },
         {
            ...
            width: 100,
            Footer: info => {
               const total = info.rows.reduce((sum, row) => row.values.score + sum, 0);
               return total;
            },
         },
      ],
   ...
   return (
      ...
      >
         <Box display={'flex'} justifyContent='space-between' mr={3} alignItems='center'>
            <Box
               display={'flex'}
               sx={{ margin: '10px' }}
               alignItems='center'
               ...
               width={800}
            >
               <Typography variant='h3'>Huddle Report</Typography>
               <Box display={'flex'}>
